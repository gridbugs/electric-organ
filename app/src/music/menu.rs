...
fn drum_loop(
    ...
    let [trig_hat_closed, trig_snare, trig_kick, ..] = bitwise_pattern_trigs_8(trig, pattern);
...
fn arp_shape(trig: impl FrameSigT<Item = bool>) -> FrameSig<impl FrameSigT<Item = ArpShape>> {
...
fn sig(channel: Channel) -> SigBoxed<f32> {
    let effects = Effects::new();
    let _hat_closed = 1 << 0;
    let snare = 1 << 1;
    let kick = 1 << 2;
    let trigger = periodic_trig_hz(3.0).build().shared();
    let drums = drum_loop(trigger.clone().divide(2), vec![kick, snare], channel);
    ...
    let melody = virtual_key_events(trigger.clone())
        ...
        .poly_voices(12)
        .into_iter()
        .map(move |mono_voice| voice(mono_voice, cutoff.clone(), resonance.clone(), channel))
        .sum::<Sig<_>>()
